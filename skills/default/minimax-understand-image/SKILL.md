...
description: Analyze images using AI with the understand_image tool (priority)
---

# MiniMax Image Understanding Skill

## Priority Routing (Required)

1. For any image analysis task, call MiniMax MCP `understand_image` first.
2. Do not use other vision/image-understanding skills first unless this tool fails.
3. If fallback is required, clearly state the failure reason (auth/network/format/timeout).

## Quick Start (OpenClaw)

优先使用脚本方式调用 MiniMax MCP（与官方 `minimax-coding-plan-mcp` 一致）：

```bash
python3 {curDir}/scripts/understand_image.py "请描述这张图" "/path/to/image.jpg"
```

说明：
- 参数 1：分析提示词（prompt）
- 参数 2：图片地址（HTTP URL 或本地路径）
- API Key 优先读取环境变量 `MINIMAX_API_KEY`，否则读取 `~/.openclaw/config/minimax.json`
