# OpenClaw 游戏化配置规范 V1（暗黑风）

> 目的：在不改动 OpenClaw 核心执行链路的前提下，将安装与配置体验升级为“职业 + 技能树 + 装备 + 等级”的游戏化体系。
...
|---|---|---|
| 角色（Class） | 职业人格（identity.role + persona files） | 7 选 1，定义默认风格与能力偏好 |
| 技能树（Skill Tree） | `skills/default/*` + 档位安装策略 | 基础/进阶/终极三层解锁 |
| 装备（Gear） | 模型、API、工具开关、安全策略 | 模型=主副武器，安全=护甲，渠道=护符 |
| 状态面板（Stats） | 服务状态、token、任务统计、成功率 | 实时显示角色战斗状态 |
| 经验与等级（XP/Level） | 使用时长+Token+任务完成+成功数 | 推动角色成长与功能解锁 |

---

## 3. 七职业定义（与当前人格系统对齐）

1. 万金油·德鲁伊：通用助手，平衡调度与执行。
2. 分析员·刺客：数据采集、价值挖掘、投研分析。
3. 研究者·大法师：论文科研、知识整理、结构化学习。
4. 管理者·召唤师：团队管理、流程治理、组织协同。
5. 技术员·战士：工程开发、测试排障、稳定性保障。
6. 营销者·圣骑士：增长运营、SEO、内容分发与转化。
7. 设计师·弓箭手：前端/UI/视觉/多媒体创作与设计交付。
...
2. 职业神殿（原 身份与个性配置）
3. 技能树祭坛（原 Skills 管理）
4. 装备工坊（原 AI模型配置 + API参数配置）
5. 传送门（原 官方消息渠道插件）
6. 圣殿法则（原 安全设置 + token规则注入）
7. 试炼场（原 快速测试）
8. 大师工坊（原 高级设置）
9. 世界之书（原 查看当前配置）
10. 营地管理（原 服务管理）
11. 修复祭坛（原 `--repair-config` 能力菜单化）
